...
conf.defaults({
  port: process.env.STATUS_PORT || 3014,
...
const servers = fs.readdirSync(path.resolve(__dirname, '../ovpnServers'))
  .filter((fname) => fname.endsWith('.json'))
  .map((fname) => JSON.parse(fs.readFileSync(path.resolve(__dirname, `./ovpnServers/${fname}`), 'utf8')))
